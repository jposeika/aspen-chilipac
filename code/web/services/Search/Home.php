...
class Search_Home extends Action {
	...
	function launch() {
		...
		if (!empty($interface->getVariable('chiliPacEnabled'))) {
			require_once ROOT_DIR . '/sys/Enrichment/ChiliPacCarousels.php';
			$chiliPacCarousels = new ChiliPacCarousels();
			$carousels = $chiliPacCarousels->getCarousels();
			$interface->assign('chiliPacRecordUrlComponent', $carousels['recordUrlComponent']);
			$interface->assign('chiliPacRecordSource', $carousels['recordSource']);
			$interface->assign('chiliPacRecentReviews', $carousels['recentReviews']);
			$interface->assign('chiliPacRecentRatings', $carousels['recentRatings']);
			$interface->assign('chiliPacHighestRated', $carousels['highestRated']);
			$interface->assign('chiliPacTrending', $carousels['trending']);
		}
