		...
		<div class="chilipac-overlay" @click.self="close">
			<div class="chilipac-dialog">
				<div class="modal-header">
					...
					<h4 class="modal-title">Add Item To Bookshelf</h4>
				</div>
...
async function doSearch(page) {
	...
	try {
		const response = await api.bookshelf().searchBibs(searchQuery.value, page);
...
async function addItem(item) {
	adding.value = true;
	try {
		await api.bookshelf().addItem(props.shelf, item.bib_id, 'mgmt');
		item.bookshelf = props.shelf;
		emit('changed');
	} finally {
		adding.value = false;
	}
...
async function removeItem(item) {
	adding.value = true;
	try {
		await api.bookshelf().removeItem(props.shelf, item.bib_id);
		item.bookshelf = null;
		emit('changed');
	} finally {
		adding.value = false;
